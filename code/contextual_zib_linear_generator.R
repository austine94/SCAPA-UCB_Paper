#contextual zero inflated burst linear scenario generator

contextual_zib_linear_generator <- function(time_horizon, K, m, n_cont, n_discrete,
  ...
  #with some probability p each entry can be zero (zero inflated burst model)
  ...
  #burst prob is the probability of an entry being replaced with a zero
  ...
  if( !is.numeric(burst_prob) | burst_prob < 0 | burst_prob > 1){
    stop("burst_prob is a numeric between zero and one")
  }
  
  if(burst_prob == 0){
    print("warning: No bursts added as probability of burst is zero")
  }
  
  if(burst_prob == 1){
    print("warning: all entries will be zero as probability of burst is one.")
  }
  ...
  #finally, add noise and zero bursts
  noise_mat <- rnorm( (time_horizon * K), 0, 1) %>% matrix(nrow = time_horizon, ncol = K)
  reward_mat <- reward_mat + noise_mat
  
  burst_vals <- sample(0:1, (time_horizon*K), replace = TRUE, prob = c(1 - burst_prob, burst_prob)) %>% as.logical()
  burst_indices <- (1:(time_horizon*K))[burst_vals]  #which entries are to be zero
  reward_mat[burst_indices] <- 0
