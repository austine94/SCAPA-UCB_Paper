#contextual poisson glm scenario generator

contextual_poisson_glm_generator <- function(time_horizon, K, m, n_cont, n_discrete,
                                             n_binary, delta_feature = 1, delta_coeff = 0.05,
                                             overlap = FALSE, training = 100){
  #function to generate random bandit scenarios with poisson glms
  ...
  training_rewards <- apply(exp(training_linear), 2, function(x) 
    rpois(training, lambda = x)) 
  training_rewards <- training_rewards + 1 #remove zeroes so transform doesn't return error
        ...
        for(i in 1:(n_changes_per_arm[k] + 1)){
          ...
          reward_mat[ ((change_times[i] + 1):change_times[i+1]), k ] <-
            apply(exp(new_linear), 2, function(x)
              rpois((change_times[i+1] - change_times[i]), lambda = x))
        }
      }
  ...
  reward_mat <- reward_mat + 1  #remove zeroes to avoid scapa returning transform error
