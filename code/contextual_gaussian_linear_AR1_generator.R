...
contextual_gaussian_linear_ar1_generator <- function(time_horizon, K, m, n_cont, n_discrete,
  ...
  #ar_coeff is the range of vals the random AR(1) coeff can take
  ...
  ar_coeff <- runif(1, ar_coeff[1], ar_coeff[2]) #generate AR coeff
  ...
  training_ar1 <- training_noise
  for(t in 2:training){ #add AR(1) noise
    training_ar1[t,] <- ar_coeff*training_ar1[(t-1),] + training_ar1[t,]
  }
  training_rewards <-  training_ar1 + training_rewards #add noise to training matrix
  ...
  for(t in 2:time_horizon){
    full_ar1[t,] <- ar_coeff*full_ar1[(t-1),] + full_ar1[t,]
  }
  
  reward_mat <- reward_mat + full_ar1
