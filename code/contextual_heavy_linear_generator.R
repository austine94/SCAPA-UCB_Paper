#contextual heavy linear scenario generator

contextual_heavy_linear_generator <- function(time_horizon, K, m, n_cont, n_discrete,
  ...
  #noise for models is heavy tailed (t with 3 degrees of freedom)
  ...
  training_noise <-  rt( (training * K), df = 3) %>% matrix(nrow = training, ncol = K)
  ...
  if(m == 0){  #if no changes to insert, just add noise
    ...
    noise_mat <- rt( (time_horizon * K), df = 3) %>% matrix(nrow = time_horizon, ncol = K)
