...
#Linear Model Fixed Drift - Scenario One
...
for(i in 1:n_reps){
  ...
  #add a Gaussian rw for drift
  rw_noise <- matrix(rnorm(K*time_horizon, 0, 0.1), nrow = time_horizon, ncol = K)
  rw_vals <- apply(rw_noise, 2, cumsum)
  data$reward_mat <- data$reward_mat + rw_vals
...
for(i in 1:n_reps){
  #generate data and fit initial model for SCAPA
  data <- contextual_gaussian_linear_ar1_generator(time_horizon, K, m, 15, 0, 5, overlap = FALSE,
                                               training = training_size, ar_coeff = c(0.1, 0.9),
                                               delta_feature = 1, delta_coeff = 3)
  model_mat <- initial_model_linear(data$training_features, data$training_rewards)
  scapa_train_reward <- training_cost(data$training_rewards, train_steps) 
...
save_kable(drift_change_table, file = "./results/Drift_Change_Results.png")
